...
async function guardarTorneos() {
  try {
    ...
    fs.writeFileSync('./data/tournaments.json', JSON.stringify(torneosFiltrados, null, 2));

    const torneoActual = torneosFiltrados.find(estaActivo);
    if (torneoActual) {
      fs.writeFileSync('./data/torneo_actual.json', JSON.stringify(torneoActual, null, 2));
    } else {
      fs.writeFileSync('./data/torneo_actual.json', JSON.stringify({ mensaje: 'No hay torneos activos' }, null, 2));
    }
