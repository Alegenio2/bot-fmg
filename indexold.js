...
const { obtenerEloActual } = require('./utils/elo.js');
const { asociarUsuario, obtenerAoeId } = require('./utils/asociar.js');
...
const fixtureJornada = require('./utils/fixtureJornada.js');
const { calcularTablaPosiciones, generarTextoTabla } = require('./utils/tablaPosiciones.js');
const { guardarTorneos } = require('./utils/guardarTorneos.js');
...
if (interaction.commandName === "admin_win") {
  ...
  if (user.id !== botConfig.directivos) {
    return interaction.reply({
    ...
    for (const jornada of liga.jornadas) {
      ...
      for (const partido of jornada.partidos) {
        ...
        if (esEstePartido) {
          ...
          if (partido.fecha || partido.horario) {
            fueRecoord = true;
          }
